...
def test_navbar(test_env, subtests):
    ...
    for key, value in WINDOWS.items():
        ...
        wait(root, 0.05)
        curr_navbar_build_test = get_navbar_build_tests(root, FONTS, key)
        window_tests.append(curr_navbar_build_test)
    
    is_navbar_window_selector = all(window_tests)
    root.update()
    ...
    all_assertions = {
        "is_navbar_window_selector": is_navbar_window_selector
    }
