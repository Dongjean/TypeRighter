...
def select_setting(selected_setting, settings_container, COLORS, FONTS):
...
def build_settings_window(root, COLORS, FONTS):
    ...
    for key, value in SETTINGS.items():
        btn = tk.Button(settings_selection_container, text=value["name"], bg=COLORS["border"], fg=COLORS["text_main"], bd=0, relief="flat", height=3, font=FONTS["font_subtitle"], activebackground=COLORS["accent_blue"], command=(lambda subwindow=key: select_setting(subwindow, settings_container, COLORS, FONTS)), name=key)
