...
def build_user_auth(root, COLORS, FONTS):
    ...
    # Login Hub Container
    login_hub_container = tk.Frame(auth_frame, bg=COLORS["bg_input"], bd=1, highlightbackground=COLORS["border"], highlightthickness=1)
    login_hub_container.pack()

    username_editor = tk.Entry(login_hub_container, bg=COLORS["bg_input"], fg=COLORS["text_main"], insertbackground="white", bd=0, font=FONTS["font_subtitle"])
    username_editor.pack(fill="both", expand=True)
    ...
    password_editor = tk.Entry(login_hub_container, bg=COLORS["bg_input"], fg=COLORS["text_main"], insertbackground="white", bd=0, font=FONTS["font_subtitle"])
    password_editor.pack(fill="both", expand=True)
    # Bind the key release event inside text_editor to our reader function
    # tk.Text() has no native function to read text in real time, this is the best option
